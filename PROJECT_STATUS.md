...
- **Base de conhecimento oficial — pendências restantes** (as duas divergências da v1.0 do COM-002 — filhos/estado civil e 4 vs. 5 cidades — foram resolvidas pelo Antonio na v1.1): (a) ainda falta definir a mensagem exata de reprovação (nota do próprio COM-002); (b) não existe ainda separação entre documento "interno" (nunca deve chegar à IA/candidata) e documento "público" no `KnowledgeDocument` — é um requisito de arquitetura para a FEATURE-003, não só um dado faltando (a seção "critério de reprovação" do COM-002 continua fora do KnowledgeEngine por causa disso, mesmo já sem conflito de discriminação).
