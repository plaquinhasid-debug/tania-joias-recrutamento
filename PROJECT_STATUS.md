...
> Atualizado em 2026-08-03 (rodada: FEATURE-003 completa — deploy da `agent-ai-gateway` feito, testada ao vivo com a IA real nas 6 perguntas de teste, 6/6 corretas). Mantido a cada mudança importante para não perder histórico entre sessões. Este arquivo descreve o estado real do código e da infraestrutura — não é um roadmap nem uma proposta.
...
- **Pipeline FEATURE-003 (`orchestrator/pipeline/answerCandidateQuestion.ts`, novo)**: liga as 4 peças acima numa função só — `KnowledgeEngine.searchByQuestion()` → `AIGateway.request()` (com os documentos encontrados como base exclusiva) → `ResponseComposer.composeResponse()`. Sem documento relevante, a IA nunca é chamada; erro/timeout da IA cai no mesmo fallback seguro do Composer, nunca propaga. Especificação aprovada e código commitado (`5dc1a0e`, `8c9f2e3`). Testado em 3 camadas: (1) exemplos locais contra a classe `AIGateway` real com providers fake (`answerCandidateQuestion.examples.ts`); (2) cenário novo no Agent Simulator (`PERGUNTAS_CONHECIMENTO`, 0 diffs); (3) **as mesmas 6 perguntas testadas ao vivo contra a Edge Function `agent-ai-gateway` já deployada (versão 4), com a Anthropic de verdade — 6/6 respostas corretas, sempre fiéis ao documento fornecido, tom batendo com o PLAYBOOK-001**. `agent-ai-gateway` foi deployada manualmente via painel do Supabase (Studio → Edge Functions → Code → colar + Deploy updates) — o deploy via ferramenta MCP direta ficou bloqueado pelo classificador de permissão do Claude Code em duas tentativas, mesmo com autorização explícita da Tania; o caminho manual funcionou de primeira depois de corrigir um erro de paste truncado. Continua shadow — nada em `useSofiaFlow.ts` chama isso ainda, e ligar ao chat real é uma decisão futura separada, que exige autorização própria.
